...
class UserInput(object):
    def __init__(self, text, validator=None, validate=True, newline=True,
                 single=False, remove_text=False, initial_input=None, check_length=True, **kw):
    ...
    def read(self):
        ...
        if input_queue:
            ...
            if self._remove_text:
                terminal.push_lock()
            terminal.push(upper, check_length=self._check_length)
...
class SpecifiedChoice(SingleCharSimpleChoice):
    ...
    def _format_choice(self, n, choice, info):
        ...
        return [' {}[{}] {}'.format(pad, n, choice)] + info
