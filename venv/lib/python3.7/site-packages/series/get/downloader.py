...
from tek_utils.sharehoster import Downloader as PutDownloader
from tek_utils.sharehoster.errors import ShareHosterError

from amino import List, Empty, Just, Path, Nothing
...
class DownloaderProc(Process, Logging):
    ...
    def _download(self):
        try:
            dl = PutDownloader(self.link.download_url, download_dir=str(self.dir))
            dl.retrieve()
...
class Downloader(ProcessHandler):
    ...
    def _create_async(self, monitor):
        ...
        if link.download_url == '':
            self._fail(monitor, link, 'download url is empty')
            return Nothing
        else:
