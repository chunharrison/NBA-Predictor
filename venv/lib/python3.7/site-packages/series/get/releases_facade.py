...
from series import make_series_name
...
class ReleasesFacade(DbFacade):
    ...
    def filter_by_metadata(self, series=None, season=None, episode=None):
        filters = List(
            Maybe.optional(series) / (lambda a: Release.name == make_series_name(a)),
            Maybe.optional(season) / (lambda a: Release.season == a),
            Maybe.optional(episode) / (lambda a: Release.episode == a),
        )
    ...
    def create(self, series, season, episode, airdate=None, search_name=None, **kw):
        ...
        release = fact(title, series, '', '', enum=enum, search_name=search_name)
        if airdate:
            release.airdate_stamp = airdate
    ...
    def http_link_by_id(self, id):
        return Maybe.check(self._db.query(Link).filter_by(id=id).first())

    def torrent_by_id(self, id):
        return Maybe.check(self._db.query(Torrent).filter_by(id=id).first())
