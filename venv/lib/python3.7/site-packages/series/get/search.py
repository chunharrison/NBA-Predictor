...
def match_title(monitor, title, res):
    ...
    attr = lambda key, target: matches.lift(key).contains(target)
    search_name = canonicalize(r.effective_search_name)
    name = canonicalize(r.name)
    canonical_title = matches.lift('title').map(canonicalize)
    return (
        (
        ...
        (
            ...
            (matches.lift('screen_size').empty and res == '')
        ) and
        (
...
class TorrentSearch(Process, Logging):
    ...
    def run(self):
        try:
            ...
            if amino.development:
                ...
                self.log.error(f'fatal error in search: {e}')
        self.done.value = 1
    ...
    def _safe_search(self, query):
        ...
        try:
            ...
            self.log.warning('Parse error in torrent search results: {}'.format(e))
        except Exception as e:
            self.log.warning('Fatal error in torrent search results: {}'.format(e))
        return List()
