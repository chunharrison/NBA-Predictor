...
from amino import List, Map, Lists
...
class LibraryFacade(Logging):
    ...
    @commit
    def _add_new_episode(self, data, new=True, tvdb_id=None):
        ...
        '''
        series = self._create_if_missing(Series, extra=dict(tvdb_id=tvdb_id), name=data.series)
        season = self._create_if_missing(Season, number=data.season, series=series)
    ...
    def _create_if_missing(self, Model, extra: dict=dict(), **params):
        ...
        return (query.first() if query.count() > 0 else self._create(Model, **params, **extra))
    ...
    def recently_watched_episodes(self, days):
        ...
        events = self._db.query(WatchEvent).filter(WatchEvent.time_begin > thresh)
        episodes = [event.episode for event in events if event.episode is not None]
        return sorted(set(episodes), reverse=True, key=lambda e: e.last_watched)

    def recently_watched_seasons(self, show, season, days):
        thresh = datetime_to_unix(datetime.now() - timedelta(days=days))
        events = (
            self._db.query(WatchEvent)
            .filter(WatchEvent.time_begin > thresh)
            .join(Episode, WatchEvent.episode_id == Episode.id)
            .join(Season, Episode.season_id == Season.id)
            .join(Series, Episode.series_id == Series.id)
            .filter(Series.name == show)
            .filter(Season.number == int(season))
        )
        return [event.episode for event in events]
    ...
    def create_episode(self, series, season, episode, tvdb_id=None):
        data = EpisodeMetadata(series, season, episode)
        return self._add_new_episode(data, tvdb_id=tvdb_id)
    ...
    def set_new(self, show, data: dict, new: bool) -> int:
        range_re = re.compile(r'^(\d+)-(\d+)$')
        params = Map(data)
        show_filter = (
            Series.id == int(show)
            if isinstance(show, int) or show.isdigit() else
            Series.name == show
        )
        def range_filter(Model: type) -> None:
            def range_filter(a: str) -> None:
                m = range_re.match(a)
                return (
                    Model.number == int(a)
                    if m is None else
                    Model.number.in_(range(int(m.group(1)), int(m.group(2)) + 1))
                )
            return range_filter
        eq = params.lift('episode').map(range_filter(Episode)).to_list
        sq = params.lift('season').map(range_filter(Season)).to_list
        flatten = lambda a: Lists.wrap(a).flat_map(Lists.wrap)
        query = lambda t, j, f, fs: flatten(self._db.query(t.id).join(j).filter(and_(f, *fs)).all())
        seasons = query(Season, Series, show_filter, sq)
        epis = query(Episode, Season, Season.id.in_(seasons), eq)
        self._db.session.execute(update(Episode).where(Episode.id.in_(epis)).values(new=new))
        self._db.commit()
        return len(epis)

    def mark_new(self, show, data: dict) -> int:
        return self.set_new(show, data, True)

    def mark_seen(self, show, data: dict) -> int:
        return self.set_new(show, data, False)
