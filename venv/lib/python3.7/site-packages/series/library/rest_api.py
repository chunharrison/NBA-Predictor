...
class RestApi(RestApiBase):
    ...
    def season_recent(self, name, season, days):
        seasons = self._library.recently_watched_seasons(name, season, days)
    ...
    def set_new(self, series, new, **kwargs):
        count = self._library.set_new(series, data=flask.request.get_json(), new=new, **kwargs)
        return (
            ('query matched nothing', 404)
            if count == 0 else
            f'updated {count} episodes'
        )

    @route('/series/<series>/mark_seen', methods=['PUT'])
    def mark_seen(self, series, **kwargs):
        return self.set_new(series, False, **kwargs)

    @route('/series/<series>/mark_new', methods=['PUT'])
    def mark_new(self, series, **kwargs):
        return self.set_new(series, True, **kwargs)
